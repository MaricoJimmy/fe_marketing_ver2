  ...
  return (
    <>
      ...
      <div className={`absolute top-0 left-0 w-full z-[60] bg-[#06101F] border-b border-white/5 py-2 px-4 overflow-hidden flex items-center justify-center transition-transform duration-500 ${hidden ? '-translate-y-full' : 'translate-y-0'}`}>
        <div className="text-xs md:text-sm font-medium text-white/70 flex items-center gap-2 group cursor-pointer hover:text-white transition-colors text-center">
          <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#22D3EE] to-[#10F0CB]">
            {t('nav.announcement.badge')}
          </span>
          <span>{t('nav.announcement.text')}</span>
          <span className="material-symbols-outlined text-sm group-hover:translate-x-1 transition-transform">arrow_outward</span>
        </div>
      </div>

      <header className={`fixed ${scrolled ? 'top-4' : 'top-12'} left-1/2 -translate-x-1/2 w-[95%] max-w-[1440px] z-50 bg-[#06101F]/70 backdrop-blur-xl border border-white/10 shadow-2xl rounded-full transition-all duration-500 ${hidden ? '-translate-y-[200%] opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="flex justify-between items-center px-4 py-1.5 md:px-6 md:py-2 lg:px-8 lg:py-2">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <HoverFillButton 
              ...
              className="bg-gradient-to-r from-[#22D3EE] to-[#10F0CB] text-[#06101F] px-4 py-1.5 md:px-5 md:py-2 rounded-full font-title-md text-sm font-bold shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all whitespace-nowrap"
            >
              {t('nav.bookDemo')}
            </HoverFillButton>
      ...
      >
        ...
        <div className="px-6 py-6 border-t border-white/10 space-y-4">
          ...
          <HoverFillButton 
            onClick={() => { navigate('/dung-thu'); setDrawerOpen(false); }}
