...
export default function BentoGrid() {
  ...
  return (
    <section id="enterprise" ref={sectionRef} className="py-xl px-margin-desktop bg-surface-container-lowest/30 backdrop-blur-sm relative overflow-hidden">
      <div className="max-w-[1440px] mx-auto space-y-xl">
        ...
        >
          ...
          <p className="font-body-md text-base md:text-lg text-on-surface-variant max-w-2xl mx-auto mt-4">{t('bento.subtitle')}</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-xl items-stretch">
            ...
            {items.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  <p className="font-body-md text-sm md:text-base text-on-surface-variant opacity-70 leading-relaxed">{t(item.descKey)}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/product/${item.id}`);
                    }}
                    className="inline-flex items-center gap-2 bg-gradient-to-r hover:from-[#22D3EE] hover:to-[#10F0CB] bg-white/5 hover:text-[#06101F] text-white px-5 py-2.5 mt-5 rounded-lg font-bold text-sm transition-all"
                  >
                    {t(item.ctaKey)}
          ...
          <div
            className="lg:col-span-7 xl:col-span-8 relative rounded-xl overflow-hidden shadow-2xl min-h-[500px] h-full bg-black/40 transition-all duration-[900ms] ease-out border border-white/5"
