...
const TextCard = ({ feature, index, total, scrollYProgress }) => {
  ...
  return (
    <motion.div 
      className="absolute w-full h-full flex flex-col items-center pt-[10%]"
      style={{ opacity, y, zIndex: 20 }}
    >
      <motion.div 
        style={{ backgroundColor, borderColor, borderWidth: '1px' }}
        className="backdrop-blur-md p-6 md:p-8 rounded-2xl md:rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.8)] text-center max-w-2xl"
      >
        <h3 className="text-white font-title-md font-bold text-2xl md:text-3xl mb-4 bg-gradient-to-r from-white to-[#10F0CB] bg-clip-text text-transparent">
          {feature.title}
        </h3>
        <p className="text-white/80 font-body-md text-sm md:text-lg leading-relaxed">
          {feature.description}
        </p>
      </motion.div>
    </motion.div>
...
export default function MacbookScroll({ features }) {
  ...
  return (
    <div ref={wrapperRef} className="relative h-[800vh] w-full mt-32 z-20">
      ...
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center w-full overflow-hidden">
        ...
        <div className="text-center px-4 w-full flex flex-col items-center justify-center mb-6 md:mb-10 z-20">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-display-md text-white font-bold tracking-tight">
            {lang === 'EN' ? 'Highlight Features' : 'Tính năng nổi bật'}
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-[#22D3EE] to-[#10F0CB] mx-auto mt-4 md:mt-6 rounded-full opacity-50"></div>
        </div>

        <div className="relative w-[95vw] md:w-[85vw] lg:w-[75vw] max-w-[1200px] aspect-[16/10] overflow-hidden flex flex-col justify-center items-center">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <AnimatePresence>
              {features.map((feature, idx) => (
                idx === activeIndex && (
                  ...
                  >
                    {feature.image ? (
                      // Đổi thành object-cover và scale nhẹ để ảnh phóng to bao quát chữ
                      <img src={feature.image} alt={feature.title} className="w-full h-full object-cover scale-105 drop-shadow-2xl opacity-100" />
                    ) : (
            ...
            {/* (Thay đổi tọa độ phần trăm % nếu ảnh Macbook của bạn có viền dày/mỏng hơn) */}
