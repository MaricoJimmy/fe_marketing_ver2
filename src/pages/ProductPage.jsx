...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.title} | Udata`;
    }
    ...
    <>
      ...
      <div className={`relative ${(['uboard', 'ugate', 'uzero'].includes(id?.toLowerCase())) ? 'pt-0' : 'min-h-screen pt-32'} pb-20 px-margin-desktop bg-[#0A0A0A]`}>
        ...
        {['uboard', 'ugate', 'uzero'].includes(id?.toLowerCase()) && (() => {
          ...
          return (
            <div className="mt-16 mb-24 max-w-[1000px] mx-auto text-center space-y-8 bg-gradient-to-b from-white/5 to-transparent p-12 md:p-16 rounded-3xl border border-white/10 relative overflow-hidden">
              ...
              <h2 className="relative z-10 text-3xl md:text-5xl font-display-md font-bold text-white leading-tight">
                {content.heading}
              </h2>
              <p className="relative z-10 text-on-surface-variant font-body-md text-lg md:text-xl max-w-3xl mx-auto leading-relaxed mt-6">
                {content.desc}
              </p>
              <div className="relative z-10 flex flex-wrap justify-center items-center gap-4 pt-10">
                <HoverFillButton 
                  onClick={() => { 
                    if (id?.toLowerCase() === 'ugate') {
                      window.location.href = 'https://ugate.udata.ai/vi/welcome'; 
