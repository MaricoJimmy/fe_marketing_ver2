        ...
        <div className="hidden lg:block">
                ...
                <button
                  ...
                  className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 ${
                    activeFilter === filter.id
                      ? activeColorClass
                      : `bg-[#0C1017] border border-white/10 text-white/70 ${hoverColorClass}`
                  }`}
                >
          ...
          <div className="grid grid-cols-3 gap-6">
            {filteredCases.map((c, i) => {
              // Find the original index for consistent images
              const originalIndex = USE_CASES_DATA.indexOf(c);
              const imageSrc = caseImages[originalIndex % caseImages.length];
              
              return (
                <ScrollReveal key={i} delay={i * 0.1}>
                  <div 
                    className="group relative bg-[#0C1017] rounded-2xl border border-white/10 overflow-hidden hover:border-[#22D3EE]/50 transition-all duration-500 hover:shadow-[0_0_30px_rgba(34,211,238,0.15)] hover:-translate-y-1 cursor-pointer flex flex-col min-h-[420px]"
                    onClick={() => setSelectedCase(c)}
                  >
                    {/* Image Header */}
                    <div className="relative h-48 overflow-hidden shrink-0">
                      <img 
                        alt={lang === 'EN' ? c.enTitle : c.viTitle} 
                        className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-70 group-hover:scale-105 transition-all duration-700" 
                        src={imageSrc} 
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-[#0C1017] via-[#0C1017]/40 to-transparent"></div>
                      
                      {/* Tags */}
                      <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                        {c.tags.map(tag => (
                          <span key={tag} className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider backdrop-blur-md border ${getTagColor(tag)}`}>
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="p-6 flex-1 flex flex-col relative z-10">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[#22D3EE] text-xs font-bold uppercase tracking-widest">{lang === 'EN' ? c.category.en : c.category.vi}</span>
                      </div>
                      <h3 className="text-xl font-bold text-white mb-3 line-clamp-2 group-hover:text-[#22D3EE] transition-colors leading-snug">
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-white/10 pt-4">
                        <span className="text-[#22D3EE] font-bold text-sm group-hover:underline">
                          {lang === 'EN' ? 'View Details' : 'Xem chi tiết'}
                        </span>
                        <span className="material-symbols-outlined text-[#22D3EE] transition-transform duration-300 group-hover:translate-x-1">arrow_forward</span>
                      </div>
                    </div>
                  </div>
                </ScrollReveal>
