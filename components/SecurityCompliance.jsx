...
export default function SecurityCompliance() {
  ...
  return (
    <section ref={sectionRef} className="py-20 px-margin-mobile md:px-margin-desktop relative overflow-hidden bg-background">
          ...
          <div className="flex w-max animate-marquee hover:[animation-play-state:paused]">
            ...
            <div className="flex">
              {items.map((item) => (
                <div 
                  key={`set1-${item.id}`} 
                  className="w-[140px] md:w-[250px] shrink-0 flex flex-col items-center justify-start md:justify-center p-4 py-6 md:p-8 border-r border-surface-border bg-surface-container-lowest/30 hover:bg-white/5 transition-colors duration-300 group"
                >
                  <div className="h-12 md:h-16 flex items-center justify-center mb-4 md:mb-6">
                    <img 
                      src={item.image} 
                      alt={t(item.titleKey)} 
                      className="max-h-full max-w-[50px] md:max-w-[80px] object-contain opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300" 
                    />
                  </div>
