  ...
  useEffect(() => {
    ...
    const autoPlayTabs = () => {
      useCaseInterval = setInterval(() => {
        if (window.innerWidth >= 768) return; // Only auto-play on mobile
        
        setUseCaseTab(prev => {
          const nextTab = prev === 4 ? 1 : prev + 1;
          
          // Scroll the container to ensure the active button is visible
          const container = useCaseTabContainerRef.current;
          if (container) {
            const buttonWidth = container.scrollWidth / 4; 
            container.scrollTo({ left: (nextTab - 1) * buttonWidth * 0.8, behavior: 'smooth' });
          }
    ...
    return () => {
      if (useCaseInterval) clearInterval(useCaseInterval);
    };
  }, []);
